...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        # icon placeholder (green flask circle)
        ...
        for col, (text, width) in enumerate([
            ...
            if width:
                self.chemicalUsed.setColumnWidth(col, width)

        self.chemicalUsed.horizontalHeader().setSectionResizeMode(
        ...
        self.chemicalUsed.verticalHeader().setDefaultSectionSize(48)
        self.chemicalUsed.verticalHeader().setSectionResizeMode(
            QtWidgets.QHeaderView.Fixed)

        # pre-fill vertical header numbers
        for i in range(5):
            item = QtWidgets.QTableWidgetItem(str(i + 1))
            item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.chemicalUsed.setVerticalHeaderItem(i, item)

        body_lay.addWidget(self.chemicalUsed)
    ...
    def retranslateUi(self, MainWindow):
        MainWindow.setWindowTitle(
            QtCore.QCoreApplication.translate("ChemicalUsedView", "Chemicals Used"))
